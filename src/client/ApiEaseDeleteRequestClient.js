const DELETE_REQUEST_PATH = 'api/v1/resources/requests';
...
class ApiEaseDeleteRequestClient {
  ...
  async deleteRequest({ apiBaseUrl, apiKey, shopDomain, requestId } = {}) {
    try {
      const response = await this.fetchImplementation(
        this.buildDeleteRequestUrl({ apiBaseUrl, requestId }),
        {
          method: 'DELETE',
  ...
  buildDeleteRequestUrl({ apiBaseUrl, requestId }) {
    return new URL(this.buildDeleteRequestPath(requestId), this.buildNormalizedBaseUrl(apiBaseUrl)).toString();
  }

  buildDeleteRequestPath(requestId) {
    return `${DELETE_REQUEST_PATH}/${encodeURIComponent(requestId)}`;
  }
