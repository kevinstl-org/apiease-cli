...
const UPDATE_REQUEST_PATH = 'api/v1/resources/requests';
...
class ApiEaseUpdateRequestClient {
  ...
  async updateRequest({ apiBaseUrl, apiKey, shopDomain, requestId, request } = {}) {
    ...
    try {
      const response = await this.fetchImplementation(
        this.buildUpdateRequestUrl({ apiBaseUrl, requestId }),
        {
          method: 'PUT',
  ...
  buildUpdateRequestUrl({ apiBaseUrl, requestId }) {
    return new URL(this.buildUpdateRequestPath(requestId), this.buildNormalizedBaseUrl(apiBaseUrl)).toString();
  }

  buildUpdateRequestPath(requestId) {
    return `${UPDATE_REQUEST_PATH}/${encodeURIComponent(requestId)}`;
  }
