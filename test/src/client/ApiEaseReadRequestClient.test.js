...
const clientModuleUrl = pathToFileURL(
  path.join(projectDirectoryPath, 'src', 'client', 'ApiEaseReadRequestClient.js'),
).href;

describe('ApiEaseReadRequestClient', () => {
  describe('constructor', () => {
    it('should be constructible for package bootstrap usage', async () => {
      // Arrange
      const { ApiEaseReadRequestClient } = await import(clientModuleUrl);

      // Act
      const apiEaseReadRequestClient = new ApiEaseReadRequestClient();

      // Assert
      assert.ok(apiEaseReadRequestClient instanceof ApiEaseReadRequestClient);
    });
  });

  describe('readRequest', () => {
    it('should get the request from the versioned requests resource item endpoint', async () => {
      // Arrange
      const { ApiEaseReadRequestClient } = await import(clientModuleUrl);
      const fetchCalls = [];
      const request = {
      ...
      // Act
      const result = await apiEaseReadRequestClient.readRequest({
    ...
    it('should not get the request from the legacy programmatic requests endpoint', async () => {
      // Arrange
      const { ApiEaseReadRequestClient } = await import(clientModuleUrl);
      const fetchCalls = [];
      const apiEaseReadRequestClient = new ApiEaseReadRequestClient({
      ...
      // Act
      await apiEaseReadRequestClient.readRequest({
        apiBaseUrl: 'https://apiease.example.com/root',
        apiKey: 'api-key-1',
        shopDomain: 'cool-shop.myshopify.com',
        requestId: 'request-2',
